...
public:
    ...
    void setContextMenu(QMenu *cmen) {
        contextM = cmen;
    }
    ...
    void removeSettings(bool permanent = false) { //such as when a plugin is deleted
        ...
        for(int i=0; i<list.length(); i++) {
            settings->remove(list[i]);
        }

    }

    void setGridGeometry(QRect grid) {
        settings->setValue(prefix+"geometry/gridLocation", grid);
    }
    QRect gridGeometry() {
        return settings->value(prefix+"geometry/gridLocation",QRect()).toRect();
    }
...
protected:
    void mousePressEvent(QMouseEvent *ev) {
        if(!dragTimer->isActive() && ev->buttons().testFlag(Qt::LeftButton) ) {
            dragTimer->start();
        }
