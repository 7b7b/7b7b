	...
	void dragMoveEvent(QDragMoveEvent *ev){
	  if(ev->mimeData()->hasFormat(MIMETYPE) ){
		...
		QPoint diff = ev->position().toPoint() - (geom.center()-QPoint(1,1)); //need difference from center (pixels)
		  ...
		  if(diff.x()<0){ geom.setLeft(ev->position().toPoint().x()/GRIDSIZE); } //expanding to the left (round down)
		  else if(diff.x()>0){ geom.setRight( ev->position().toPoint().x()/GRIDSIZE); } //expanding to the right (round down)
		  if(diff.y()<0){ geom.setTop( ev->position().toPoint().y()/GRIDSIZE); } //expanding above  (round down)
		  else if(diff.y()>0){ geom.setBottom( ev->position().toPoint().y()/GRIDSIZE); } //expanding below (round down)
