...
void page_interface_panels::setupProfiles() {
    ...
    for(int p=0; p<2; p++) {
        if(p==1) {
            profiles = profilesAll;    //use whats left of the total list
        }
        ui->tool_profile->menu()->addSection( p==0 ? tr("Custom Profiles") : tr("Copy Screen") );
        for(int i=0; i<profiles.length(); i++) {
            if(p==0) {
                profilesAll.removeAll(profiles[i]);    //handling it now
            }
            QString title = profiles[i];
            if(title.startsWith("profile_")) {
                title = title.section("profile_",-1);
            }
    ...
    if(wt.startsWith("profile_")) {
        ...
        if(wt=="profile_new") {
            ...
            for(int i=0; i<keys.length(); i++) {
                if(keys[i].section("/",0,0).contains(pname)) {
