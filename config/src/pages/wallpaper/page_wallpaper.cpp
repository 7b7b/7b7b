...
void page_wallpaper::SaveSettings() {
    ...
    if(bgs.isEmpty()) {
        bgs << "default";    //Make sure to always fall back on the default
    }
...
void page_wallpaper::LoadSettings(int screennum) {
    ...
    for(int i=0; i<bgs.length(); i++) {
        ...
        //else{ ui->combo_desk_bg->addItem( QIcon(QPixmap(bgs[i]).scaled(64,64)), bgs[i].section("/",-1), bgs[i] ); }
            ...
            ui->combo_desk_bg->addItem( bgs[i].section("/",-1), bgs[i] );    //disable the thumbnail - takes a long time for large collections of files
        }
    }
    ...
    if(tmp>=0) {
        ui->combo_desk_layout->setCurrentIndex(tmp);
    }
    loading = false;
}
...
void page_wallpaper::updateMenus() {
    ...
    if(ui->tool_desk_addbg->menu()==0) {
        ui->tool_desk_addbg->setMenu(new QMenu(this));
    }
