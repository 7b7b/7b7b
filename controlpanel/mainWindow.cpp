...
void mainWindow::on_actionBack_triggered() {
    if(cpage.isEmpty()) {
        static_cast<page_main*>( this->centralWidget() )->clearlineEdit();
    }
    else {
        page_change("");    //Use the interactive wrapper (check for save state, etc).
    }
}
...
void mainWindow::changeMonitor(QAction *act) {
    ...
    if( page_change(cpage) ) {
        ui->actionMonitor->setText(act->text());
    }
    else {
        ui->actionMonitor->setWhatsThis(oldWT);    //cancelled - go back to old setting
    }
}
