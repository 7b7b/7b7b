...
PAGEINFO Pages::PageInfo(QString ID, QString i_name, QString i_title, QString i_icon, QString i_comment, QString i_cat, QStringList i_sys, QStringList i_tags) {
    ...
    page.title = i_title;
    page.icon = i_icon;
    page.comment = i_comment;
    page.category = i_cat;
    page.req_systems = i_sys;
...
PageWidget* Pages::GetNewPage(QString id, QWidget *parent) {
    ...
    PageWidget* page = 0;
    if(id=="wallpaper") {
        page = new page_wallpaper(parent);
    }
    else if(id=="autostart") {
        page = new page_autostart(parent);
    }
    else if(id=="defaultapps") {
        page = new page_defaultapps(parent);
    }
    else if(id=="interface-desktop") {
        page = new page_interface_desktop(parent);
    }
    else if(id=="interface-panel") {
        page = new page_interface_panels(parent);
    }
    else if(id=="interface-menu") {
        page = new page_interface_menu(parent);
    }
    else if(id=="session-locale") {
        page = new page_session_locale(parent);
    }
    else if(id=="session-options") {
        page = new page_session_options(parent);
    }
    //Return the main control_panel page as the fallback/default
    if(page==0) {
        id.clear();
        page = new page_main(parent);
    }
