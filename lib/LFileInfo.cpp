  ...
  if(zfs_ds.isEmpty()){
    ...
    if(ok){
      ...
      if(!perms.isEmpty() && ok){
        ...
        if(!gplist.isEmpty()){ gplist = gplist.first().replace("\t", " ").split(" ",Qt::SkipEmptyParts); gplist.removeAll("groups"); }
        for(int i=0; i<gplist.length(); i++){
          //QStringList tmp = perms.filter(QRegExp("[user|group] "+gplist[i]));
          QStringList tmp = perms.filter(QRegularExpression("[user|group] "+gplist[i]));
          if(tmp.isEmpty()){ continue; }
          zfs_perms << tmp.first().section(" ",2,2,QString::SectionSkipEmpty).split(",",Qt::SkipEmptyParts);
        }
