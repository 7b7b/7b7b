...
void XDGDesktop::sync(){
  ...
  for(int i=0; i<file.length(); i++){
    ...
    else if(var=="Categories" && insection){ catList = val.split(";",Qt::SkipEmptyParts); }
    else if(var=="OnlyShowIn" && insection){ showInList = val.split(";",Qt::SkipEmptyParts); }
    else if(var=="NotShowIn" && insection){ notShowInList = val.split(";",Qt::SkipEmptyParts); }
    else if(var=="Terminal" && insection){ useTerminal= (val.toLower()=="true"); }
    else if(var=="Actions" && insection){ actionList = val.split(";",Qt::SkipEmptyParts); }
    else if(var=="MimeType" && insection){ mimeList = val.split(";",Qt::SkipEmptyParts); }
    else if(var=="Keywords" && insection){
      if(keyList.isEmpty() && loc.isEmpty()){ keyList = val.split(";",Qt::SkipEmptyParts); }
      else if(loc == lang){ keyList = val.split(";",Qt::SkipEmptyParts); }
    }
