...
void DesktopSettings::start() {
    files.clear();
    settings.clear(); //clear the internal hashes (just in case)
    if(watcher==0) {
...
void DesktopSettings::stop() {
    QStringList watch;
    watch << watcher->files() << watcher->directories();
    if(!watch.isEmpty()) {
        watcher->removePaths(watch);
    }
...
void DesktopSettings::parseSystemSettings() {
    ...
    for(int i=0; i<dirs.length(); i++) {
        if(dirs[i].endsWith("/xdg")) {
            dirs[i] = dirs[i].section("/",0,-2);    //Chop off the xdg subdir - need the generic configuration directory
        }
        ...
        if(defMode=="fullsystem") {
            runmode= DesktopSettings::SystemFull;
        }
        else if(defMode=="staticinterface") {
            runmode = DesktopSettings::SystemInterface;
        }
        else {
            runmode = DesktopSettings::UserFull;
        }
        ...
        if(pw!=0) {
            ...
            if( settings[path]->value("fulluser_users", QStringList()).toStringList().contains(cuser) ) {
                runmode = DesktopSettings::UserFull;
            }
            else if( settings[path]->value("fullsystem_users", QStringList()).toStringList().contains(cuser) ) {
                runmode = DesktopSettings::SystemFull;
            }
            else if( settings[path]->value("staticinterface_users", QStringList()).toStringList().contains(cuser) ) {
                runmode = DesktopSettings::SystemInterface;
            }
            else {
                ...
                if( getgrouplist(cuser.toLocal8Bit(), getgid(), grpList, &grpSize) > 0 ) {
                    ...
                    if( (fromfile+groups).removeDuplicates() > 0 ) {
                        runmode = DesktopSettings::UserFull;
                    }
                    else {
    ...
    QString mode;
    if(runmode == DesktopSettings::UserFull) {
        mode = "Full User";
    }
    else if(runmode == DesktopSettings::SystemFull) {
        mode = "Full System";
    }
    ...
    for(int i=0; i<systemdirs.length(); i++) {
        ...
        for(int j=0; j<tmp.length(); j++) {
            ...
            if(QFile::exists(path)) {
                ...
                if(files.contains(tmp[j])) {
                    filepaths = files[tmp[j]];
                }
