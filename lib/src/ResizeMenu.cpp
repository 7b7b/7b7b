...
void ResizeMenu::mouseMoveEvent(QMouseEvent *ev) {
    ...
    switch(resizeSide) {
    case TOP:
        if(gpos.y() >= geom.bottom()-1) {
            break;
        }
        geom.setTop(gpos.y());
        this->setGeometry(geom);
        if(contents!=0) {
            contents->setFixedSize(QSize(geom.width()-2, geom.height()-2));
        }
        handled = true;
        break;
    case BOTTOM:
        if(gpos.y() <= geom.top()+1) {
            break;
        }
    ...
    default: //NONE
        ...
        if(ev->pos().x()<=1 && ev->pos().x() >= -1) {
            this->setCursor(Qt::SizeHorCursor);
        }
        else if(ev->pos().x() >= this->width()-1 && ev->pos().x() <= this->width()+1) {
            this->setCursor(Qt::SizeHorCursor);
        }
        else if(ev->pos().y()<=1 && ev->pos().y() >= -1) {
            this->setCursor(Qt::SizeVerCursor);
        }
        else if(ev->pos().y() >= this->height()-1 && ev->pos().y() <= this->height()+1) {
            this->setCursor(Qt::SizeVerCursor);
        }
        else {
...
void ResizeMenu::mousePressEvent(QMouseEvent *ev) {
    ...
    if(used) {
        ...
        QMenu::mousePressEvent(ev);    //do normal processing
    }
}
