...
QList<WId> LXCB::WindowList(bool rawlist) {
    ...
    if( 1 == xcb_ewmh_get_client_list_reply( &EWMH, cookie, &winlist, NULL) ) {
        ...
        for(unsigned int i=0; i<winlist.windows_len; i++) {
            ...
            else if( (WindowWorkspace(winlist.windows[i])!=wkspace) && !rawlist ) {
                continue;
            }
            else {
...
unsigned int LXCB::WindowWorkspace(WId win) {
    if(DEBUG) {
        qDebug() << "XCB: WindowWorkspace()" << win;
    }
    ...
    if(1==xcb_ewmh_get_wm_state_reply(&EWMH,scookie, &reply, NULL)) {
        ...
        for(unsigned int i=0; i<reply.atoms_len; i++) {
            if(reply.atoms[i]==EWMH._NET_WM_STATE_STICKY) {
                wkspace = LXCB::CurrentWorkspace();
                break;
            }
        }
    }
...
QList<int> LXCB::WindowFrameGeometry(WId win) {
    if(DEBUG) {
        qDebug() << "XCB: WindowFrameGeometry()";
    }
...
LXCB::WINDOWVISIBILITY LXCB::WindowState(WId win) {
    ...
    if( 1 == xcb_ewmh_get_wm_state_reply(&EWMH, stateCookie, &states, NULL) ) {
        for(unsigned int i=0; i<states.atoms_len; i++) {
            ...
            else if(states.atoms[i] == EWMH._NET_WM_STATE_HIDDEN) {
                cstate = INVISIBLE;
            }
        }
    }
...
QString LXCB::WindowVisibleIconName(WId win) { //_NET_WM_VISIBLE_ICON_NAME
    if(DEBUG) {
        qDebug() << "XCB: WindowVisibleIconName()";
    }
    if(win==0) {
        return "";
    }
...
void LXCB::SetDisableWMActions(WId win) {
    if(DEBUG) {
        qDebug() << "XCB: SetDisableWMActions()";
    }
...
void LXCB::MinimizeWindow(WId win) { //request that the window be unmapped/minimized
    if(DEBUG) {
        qDebug() << "XCB: MinimizeWindow()";
    }
    if(win==0) {
        return;
    }
    if(atoms.isEmpty()) {
        createWMAtoms();    //need these atoms
    }
...
void LXCB::ActivateWindow(WId win) { //request that the window become active
    ...
    if(actwin == win) {
        return;    //requested window is already active
    }
...
uint LXCB::EmbedWindow(WId win, WId container) {
    if(DEBUG) {
        qDebug() << "XCB: EmbedWindow()";
    }
    //This returns the damage control ID number (or 0 for a failure)
    if(win==0 || container==0 || LXCB::WindowClass(win).isEmpty() ) {
        return 0;    //invalid window (destroyed before getting here?)
    }
    ...
    if(ereply==0) {
        return 0;    //unable to initialize the atom
    }
...
#define CLIENT_WIN_EVENT_MASK (XCB_EVENT_MASK_PROPERTY_CHANGE |  \
    ...
    if(WM_ICCCM_GetClass(win).isEmpty() ) {
        return false;
    }
    xcb_get_window_attributes_cookie_t cookie = xcb_get_window_attributes(QX11Info::connection(), win);
    xcb_get_window_attributes_reply_t *attr = xcb_get_window_attributes_reply(QX11Info::connection(), cookie, NULL);
    if(attr == 0) {
        return false;    //could not get attributes of window
    }
    if(attr->override_redirect) {
        free(attr);    //window has override redirect set (do not manage)
        return false;
    }
...
icccm_size_hints LXCB::WM_ICCCM_GetSizeHints(WId win) {
    ...
    if(1==xcb_icccm_get_wm_size_hints_reply(QX11Info::connection(), cookie, &reply, NULL) ) {
        ...
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_US_POSITION)==XCB_ICCCM_SIZE_HINT_US_POSITION ) {
            hints.x=reply.x;
            hints.y=reply.y;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_US_SIZE)==XCB_ICCCM_SIZE_HINT_US_SIZE ) {
            hints.width=reply.width;
            hints.height=reply.height;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_POSITION)==XCB_ICCCM_SIZE_HINT_P_POSITION ) {
            hints.x=reply.x;
            hints.y=reply.y;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_SIZE)==XCB_ICCCM_SIZE_HINT_P_SIZE ) {
            hints.width=reply.width;
            hints.height=reply.height;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_MIN_SIZE)==XCB_ICCCM_SIZE_HINT_P_MIN_SIZE ) {
            hints.min_width=reply.min_width;
            hints.min_height=reply.min_height;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_MAX_SIZE)==XCB_ICCCM_SIZE_HINT_P_MAX_SIZE ) {
            hints.max_width=reply.max_width;
            hints.max_height=reply.max_height;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_RESIZE_INC)==XCB_ICCCM_SIZE_HINT_P_RESIZE_INC ) {
            hints.width_inc=reply.width_inc;
            hints.height_inc=reply.height_inc;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_ASPECT)==XCB_ICCCM_SIZE_HINT_P_ASPECT ) {
            hints.min_aspect_num=reply.min_aspect_num;
            hints.min_aspect_den=reply.min_aspect_den;
            hints.max_aspect_num=reply.max_aspect_num;
            hints.max_aspect_den=reply.max_aspect_den;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_BASE_SIZE)==XCB_ICCCM_SIZE_HINT_BASE_SIZE ) {
            hints.base_width=reply.base_width;
            hints.base_height=reply.base_height;
        }
        if( (reply.flags&XCB_ICCCM_SIZE_HINT_P_WIN_GRAVITY)==XCB_ICCCM_SIZE_HINT_P_WIN_GRAVITY ) {
            hints.win_gravity=reply.win_gravity;
        }
        //free(reply);
    }
    return hints;
...
void LXCB::WM_Set_Workarea(QList<QRect> list) {
    ...
    for(int i=0; i<list.length(); i++) {
        array[i].x = list[i].x();
        array[i].y = list[i].y();
        array[i].width = list[i].width();
        array[i].height = list[i].height();
    }
    //Now set the property
...
void LXCB::WM_Set_Desktop(WId win, int num) {
    ...
    if(num<0) {
        num = 0;    //TEMPORARY FIX - Fluxbox does not like negative numbers any more?
    }
...
QList<LXCB::WINDOWTYPE> LXCB::WM_Get_Window_Type(WId win) {
    ...
    if(1==xcb_ewmh_get_wm_window_type_reply(&EWMH, cookie, &reply, NULL) ) {
        for(unsigned int i=0; i<reply.atoms_len; i++) {
            if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_DESKTOP) {
                out << LXCB::T_DESKTOP;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_DOCK) {
                out << LXCB::T_DOCK;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_TOOLBAR) {
                out << LXCB::T_TOOLBAR;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_MENU) {
                out << LXCB::T_MENU;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_UTILITY) {
                out << LXCB::T_UTILITY;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_SPLASH) {
                out << LXCB::T_SPLASH;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_DIALOG) {
                out << LXCB::T_DIALOG;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_DROPDOWN_MENU) {
                out << LXCB::T_DROPDOWN_MENU;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_POPUP_MENU) {
                out << LXCB::T_POPUP_MENU;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_TOOLTIP) {
                out << LXCB::T_TOOLTIP;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_NOTIFICATION) {
                out << LXCB::T_NOTIFICATION;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_COMBO) {
                out << LXCB::T_COMBO;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_DND) {
                out << LXCB::T_DND;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_WINDOW_TYPE_NORMAL) {
                out << LXCB::T_NORMAL;
            }
        }
    }
...
void LXCB::WM_Set_Window_Type(WId win, QList<LXCB::WINDOWTYPE> list) {
    ...
    for(int i=0; i<list.length(); i++) {
        switch(list[i]) {
        case LXCB::T_DESKTOP:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_DESKTOP;
            break;
        case LXCB::T_DOCK:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_DOCK;
            break;
        case LXCB::T_TOOLBAR:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_TOOLBAR;
            break;
        case LXCB::T_MENU:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_MENU;
            break;
        case LXCB::T_UTILITY:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_UTILITY;
            break;
        case LXCB::T_SPLASH:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_SPLASH;
            break;
        case LXCB::T_DIALOG:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_DIALOG;
            break;
        case LXCB::T_DROPDOWN_MENU:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_DROPDOWN_MENU;
            break;
        case LXCB::T_POPUP_MENU:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_POPUP_MENU;
            break;
        case LXCB::T_TOOLTIP:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_TOOLTIP;
            break;
        case LXCB::T_NOTIFICATION:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_NOTIFICATION;
            break;
        case LXCB::T_COMBO:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_COMBO;
            break;
        case LXCB::T_DND:
            array[i] = EWMH._NET_WM_WINDOW_TYPE_DND;
            break;
        default:
...
QList<LXCB::WINDOWSTATE> LXCB::WM_Get_Window_States(WId win) {
    ...
    if(1==xcb_ewmh_get_wm_state_reply(&EWMH, cookie, &reply, NULL) ) {
        for(unsigned int i=0; i<reply.atoms_len; i++) {
            if(reply.atoms[i]==EWMH._NET_WM_STATE_MODAL) {
                out << LXCB::S_MODAL;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_STICKY) {
                out << LXCB::S_STICKY;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_MAXIMIZED_VERT) {
                out << LXCB::S_MAX_VERT;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_MAXIMIZED_HORZ) {
                out << LXCB::S_MAX_HORZ;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_SHADED) {
                out << LXCB::S_SHADED;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_SKIP_TASKBAR) {
                out << LXCB::S_SKIP_TASKBAR;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_SKIP_PAGER) {
                out << LXCB::S_SKIP_PAGER;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_HIDDEN) {
                out << LXCB::S_HIDDEN;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_FULLSCREEN) {
                out << LXCB::S_FULLSCREEN;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_ABOVE) {
                out << LXCB::S_ABOVE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_BELOW) {
                out << LXCB::S_BELOW;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_STATE_DEMANDS_ATTENTION) {
                out << LXCB::S_ATTENTION;
            }
...
void LXCB::WM_Set_Window_States(WId win, QList<LXCB::WINDOWSTATE> list) {
    ...
    for(int i=0; i<list.length(); i++) {
        switch(list[i]) {
        case LXCB::S_MODAL:
            array[i] = EWMH._NET_WM_STATE_MODAL;
            break;
        case LXCB::S_STICKY:
            array[i] = EWMH._NET_WM_STATE_STICKY;
            break;
        case LXCB::S_MAX_VERT:
            array[i] = EWMH._NET_WM_STATE_MAXIMIZED_VERT;
            break;
        case LXCB::S_MAX_HORZ:
            array[i] = EWMH._NET_WM_STATE_MAXIMIZED_HORZ;
            break;
        case LXCB::S_SHADED:
            array[i] = EWMH._NET_WM_STATE_SHADED;
            break;
        case LXCB::S_SKIP_TASKBAR:
            array[i] = EWMH._NET_WM_STATE_SKIP_TASKBAR;
            break;
        case LXCB::S_SKIP_PAGER:
            array[i] = EWMH._NET_WM_STATE_SKIP_PAGER;
            break;
        case LXCB::S_HIDDEN:
            array[i] = EWMH._NET_WM_STATE_HIDDEN;
            break;
        case LXCB::S_FULLSCREEN:
            array[i] = EWMH._NET_WM_STATE_FULLSCREEN;
            break;
        case LXCB::S_ABOVE:
            array[i] = EWMH._NET_WM_STATE_ABOVE;
            break;
        case LXCB::S_BELOW:
            array[i] = EWMH._NET_WM_STATE_BELOW;
            break;
        case LXCB::S_ATTENTION:
            array[i] = EWMH._NET_WM_STATE_DEMANDS_ATTENTION;
            break;
            //case LXCB::FOCUSED:
...
QList<LXCB::WINDOWACTION> LXCB::WM_Get_Window_Actions(WId win) {
    ...
    if(1==xcb_ewmh_get_wm_allowed_actions_reply(&EWMH, cookie, &reply, NULL) ) {
        for(unsigned int i=0; i<reply.atoms_len; i++) {
            if(reply.atoms[i]==EWMH._NET_WM_ACTION_MOVE) {
                out << LXCB::A_MOVE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_RESIZE) {
                out << LXCB::A_RESIZE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_MINIMIZE) {
                out << LXCB::A_MINIMIZE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_SHADE) {
                out << LXCB::A_SHADE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_STICK) {
                out << LXCB::A_STICK;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_MAXIMIZE_HORZ) {
                out << LXCB::A_MAX_HORZ;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_MAXIMIZE_VERT) {
                out << LXCB::A_MAX_VERT;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_FULLSCREEN) {
                out << LXCB::A_FULLSCREEN;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_CHANGE_DESKTOP) {
                out << LXCB::A_CHANGE_DESKTOP;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_CLOSE) {
                out << LXCB::A_CLOSE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_ABOVE) {
                out << LXCB::A_ABOVE;
            }
            else if(reply.atoms[i]==EWMH._NET_WM_ACTION_BELOW) {
                out << LXCB::A_BELOW;
            }
        }
    }
...
void LXCB::WM_Set_Window_Actions(WId win, QList<LXCB::WINDOWACTION> list) {
    ...
    for(int i=0; i<list.length(); i++) {
        switch(list[i]) {
        case LXCB::A_MOVE:
            array[i] = EWMH._NET_WM_ACTION_MOVE;
            break;
        case LXCB::A_RESIZE:
            array[i] = EWMH._NET_WM_ACTION_RESIZE;
            break;
        case LXCB::A_MINIMIZE:
            array[i] = EWMH._NET_WM_ACTION_MINIMIZE;
            break;
        case LXCB::A_SHADE:
            array[i] = EWMH._NET_WM_ACTION_SHADE;
            break;
        case LXCB::A_STICK:
            array[i] = EWMH._NET_WM_ACTION_STICK;
            break;
        case LXCB::A_MAX_HORZ:
            array[i] = EWMH._NET_WM_ACTION_MAXIMIZE_HORZ;
            break;
        case LXCB::A_MAX_VERT:
            array[i] = EWMH._NET_WM_ACTION_MAXIMIZE_VERT;
            break;
        case LXCB::A_FULLSCREEN:
            array[i] = EWMH._NET_WM_ACTION_FULLSCREEN;
            break;
        case LXCB::A_CHANGE_DESKTOP:
            array[i] = EWMH._NET_WM_ACTION_CHANGE_DESKTOP;
            break;
        case LXCB::A_CLOSE:
            array[i] = EWMH._NET_WM_ACTION_CLOSE;
            break;
        case LXCB::A_ABOVE:
            array[i] = EWMH._NET_WM_ACTION_ABOVE;
            break;
        case LXCB::A_BELOW:
            array[i] = EWMH._NET_WM_ACTION_BELOW;
            break;
        }
    }
    //Now set the property
...
QList<unsigned int> LXCB::WM_Get_Window_Strut(WId win) {
    ...
    out << 0 << 0 << 0 << 0; //init the output list
...
QList<strut_geom> LXCB::WM_Get_Window_Strut_Partial(WId win) {
    ...
    QList<strut_geom> out;
    out << strut_geom() << strut_geom() << strut_geom() << strut_geom();
    ...
    if(1==xcb_ewmh_get_wm_strut_partial_reply(&EWMH, cookie, &reply, NULL) ) {
        if(reply.left>0) {
            out[0].start = reply.left_start_y;
            out[0].end = reply.left_end_y;
            out[0].thick = reply.left;
        }
        if(reply.right>0) {
            out[1].start = reply.right_start_y;
            out[1].end = reply.right_end_y;
            out[1].thick = reply.right;
        }
        if(reply.top>0) {
            out[2].start = reply.top_start_x;
            out[2].end = reply.top_end_x;
            out[2].thick = reply.top;
        }
        if(reply.bottom>0) {
            out[3].start = reply.bottom_start_x;
            out[3].end = reply.bottom_end_x;
            out[3].thick = reply.bottom;
        }
    }
    return out;
}
...
void LXCB::WM_Set_Window_Strut_Partial(WId win, QList<strut_geom> struts) {
    //Input: [left,right,top,bottom] - must be length 4
    while(struts.length() < 4) {
        struts << strut_geom();
    }
    //Convert to the XCB input format
    xcb_ewmh_wm_strut_partial_t input;
    input.left=struts[0].thick;
    input.left_start_y=struts[0].start;
    input.left_end_y=struts[0].end;
    input.right=struts[1].thick;
    input.right_start_y=struts[1].start;
    input.right_end_y=struts[1].end;
    input.top=struts[2].thick;
    input.top_start_x=struts[2].start;
    input.top_end_x=struts[2].end;
    input.bottom=struts[3].thick;
    input.bottom_start_x=struts[3].start;
    input.bottom_end_x=struts[3].end;
    //Now set the property
    ...
    if(1==xcb_ewmh_get_wm_icon_reply(&EWMH, cookie, &reply, NULL) ) {
        ...
        while(it.index < reply.num_icons) {
            ...
            if(it.rem>0) {
                ...
                break;    //just finished the last one - ensure this breaks out now (just in case)
            }
        }
...
QList<unsigned int> LXCB::WM_Get_Fullscreen_Monitors(WId win) {
    ...
    QList<unsigned int> out;
    out << 0 << 0 << 0 << 0; //init the output array
