...
inline QStringList atomsToNames(xcb_atom_t *atoms, unsigned int num) {
    ...
    for(unsigned int i=0; i<num; i++) {
        cookies << xcb_get_atom_name_unchecked(QX11Info::connection(), atoms[i]);
    }
    QStringList names;
    //qDebug() << " - Get names";
...
inline bool loadScreenInfo(p_objects *p_obj) {
    ...
    xcb_randr_get_output_info_reply_t *info = xcb_randr_get_output_info_reply(QX11Info::connection(),
            ...
            NULL);
    if(info==0) {
        return false;    //bad output value
    }
    //First read off the information associated with the output itself
    if(p_obj->name.isEmpty()) {
        p_obj->name = QString::fromLocal8Bit( (char*) xcb_randr_get_output_info_name(info), xcb_randr_get_output_info_name_length(info));
    }
        ...
        if(srreply!=0) {
            for(int i=0; i<xcb_randr_get_screen_resources_modes_length(srreply); i++) {
                ...
                if(p_obj->modes.contains(minfo.id)) {
                    ...
                    if(!p_obj->resolutions.contains(sz)) {
                        p_obj->resolutions.append( sz);
                    }
                }
            }
            free(srreply);
        ...
        for(int i=0; i<xcb_randr_get_screen_resources_modes_length(srreply); i++) {
            ...
            if(modes.contains(minfo.id)) {
                ...
                if(res.isNull() && (minfo.width > sz.width() || minfo.height > sz.height()) ) {
                    ...
                    if(sz == res && minfo.dot_clock > refreshrate) {
                        det_mode = minfo.id;
                        refreshrate = minfo.dot_clock;
                    }
                }
            }
        }
...
inline bool showOutput(QRect geom, p_objects *p_obj) {
    ...
    if(mode==XCB_NONE) {
        qDebug() << "[ERROR] Invalid resolution supplied!";    //invalid resolution for this monitor
        return false;
    }
        ...
        for(int i=0; i<num && p_obj->crtc==0; i++) {
            ...
            if(xcb_randr_get_crtc_info_outputs_length(info) < 1) { //make sure it is not already associated with an output
                ...
                for(int p=0; p<pnum; p++) {
                    possible << xcb_randr_get_crtc_info_possible(info)[p];
                }
            }
            if(possible.contains(p_obj->output)) {
                p_obj->crtc = crt;
            }
            free(info);
        }
    ...
    if(p_obj->crtc == 0) {
        qDebug() << "[ERROR] No Available CRTC devices for display";
        return false;
    }
    ...
    bool ok = false;
    if(reply!=0) {
        ok = (reply->status == XCB_RANDR_SET_CONFIG_SUCCESS);
    }
    free(reply);
    return ok;
}
...
QString OutputDevice::ID() {
    return p_obj.name;
}
bool OutputDevice::isEnabled() {
    return !p_obj.geometry.isNull();
}
bool OutputDevice::isPrimary() {
    return p_obj.primary;
}
bool OutputDevice::isConnected() {
    return !p_obj.modes.isEmpty();
}

QList<QSize> OutputDevice::availableResolutions() {
    return p_obj.resolutions;
}
QSize OutputDevice::currentResolution() {
    return p_obj.geometry.size();    //no concept of panning/scaling yet
}
QRect OutputDevice::currentGeometry() {
    return p_obj.geometry;
}
QSize OutputDevice::physicalSizeMM() {
    return p_obj.physicalSizeMM;
}
...
bool OutputDevice::setAsPrimary(bool set) {
    if(p_obj.primary == set) {
        return true;    //no change needed
    }
    if(set) {
        xcb_randr_set_output_primary (QX11Info::connection(), QX11Info::appRootWindow(), p_obj.output);
    }
...
OutputDeviceList::OutputDeviceList() {
    xcb_randr_get_screen_resources_reply_t *reply = xcb_randr_get_screen_resources_reply(QX11Info::connection(),
            ...
            NULL);
    if(reply==0) {
        return;    //could not get screen information
    }
    int outputnum = xcb_randr_get_screen_resources_outputs_length(reply);
    for(int i=0; i<outputnum; i++) {
        xcb_randr_output_t output = xcb_randr_get_screen_resources_outputs(reply)[i];
...
QString OutputDeviceList::primaryMonitor() {
    for(int i=0; i<out_devs.length(); i++) {
        if(out_devs[i].isPrimary()) {
            return out_devs[i].ID();
        }
    }
    return "";
}
